...
async function api(path, options = {}) {
    ...
    // 走 catch-all 代理 /api/proxy/<path>：
    //   api('/api/login.php')                → /api/proxy/login
    //   api('/api/partner.php?action=bind')  → /api/proxy/partner?action=bind
    const trimmed = path.replace(/^\/+|\/+$/g, '');
    const cleanPath = trimmed.replace(/^api\//, '').replace(/\.php(\?|$)/, '$1');
    const qIdx = cleanPath.indexOf('?');
    const targetPath = qIdx >= 0 ? cleanPath.slice(0, qIdx) : cleanPath;
    const qStr = qIdx >= 0 ? cleanPath.slice(qIdx + 1) : '';
    const url = '/api/proxy/' + targetPath + (qStr ? '?' + qStr : '');
